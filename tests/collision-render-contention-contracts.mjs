...
for(const [status,active,lost,preparing,expected] of [
 ...
 ['preparing','shaders',false,false,1],
 ['ready',null,false,false,1],
 ['failed','collision',false,false,0],
 ['preparing','collision',true,false,0],
 ['ready',null,true,false,0],
 ['ready',null,false,true,0],
]){
 const calls={reset:0,draw:0,raf:0},metrics={};const continuation=()=>{};
 present({game:{contextLost:lost},graphicsPreparation:preparing,loadingSnapshot:()=>({status,active}),
 renderer:{info:{reset(){calls.reset++;},render:{calls:7,triangles:42,points:3}}},
 cinematic:{render(){calls.draw++;}},frameMetrics:metrics,render:continuation,
 requestAnimationFrame(fn){assert.equal(fn,continuation);calls.raf++;}});
 assert.deepEqual(calls,{reset:expected,draw:expected,raf:1},JSON.stringify({status,active,lost,preparing}));
...
checks++;
console.log(JSON.stringify({passed:true,checks,scope:'Actual presentation branch: collision defers draw, later stages resume draw, rAF continues, original deadlines retained. No performance or Safari claim.'}));
