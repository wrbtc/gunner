...
assert.match(html,/styles\.css\?v=054-84/);
assert.match(html,/mobile-entry\.js\?v=desktop-054-87/);
assert.match(html,/data-game-entry="\.\/main\.js\?v=[A-Za-z0-9.-]+"/);
...
assert.match(read('src/mission-screen.js'),/export const REPORT_BUILD='0\.54\.88'/);
...
assert.match(read('mobile-entry.js'),/mission-screen\.js\?v=054-87/);
assert.match(main,/updateFps\(now\)/);
assert.doesNotMatch(main,/Intel Reduced present/);
assert.doesNotMatch(main,/if\(!preferences\.reduced\)updateFps/);
