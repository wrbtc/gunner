...
function graphicsFixture({ready=false,compileMs=0,uniformMs=0,attributeMs=0}={}){
 ...
 const prepare=timeoutMs=>pass.prepare(scene,camera,{timeoutMs,onProgress:event=>events.push(event)});
...
await check('failure-report-preserves-only-bounded-typed-graphics-evidence',async()=>{
 ...
 try{
  ...
  assert.equal(report.build,'0.54.29');const snapshot=report.loading.progress.preTimeoutSnapshot;
