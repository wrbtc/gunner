...
assert.deepEqual(render(),{calls:16,world:2,clears:1});assert.equal(cinema.stats().worldMsaaSamples,4);assert.equal(cinema.stats().contactOcclusionSamples,8);
...
assert.deepEqual(render({reducedEffects:true}),{calls:6,world:2,clears:1});assert.equal(disposals,0);assert.equal(cinema.stats().worldMsaaSamples,4);assert.equal(cinema.stats().contactOcclusionSamples,0);assert.equal(hdr.depthTexture,depth);assert.equal(hdr.texture,texture);
render({reducedEffects:true});assert.equal(disposals,0,'stable profile must not reallocate per frame');
assert.deepEqual(render(),{calls:16,world:2,clears:1});assert.equal(disposals,0);assert.equal(cinema.stats().worldMsaaSamples,4);assert.equal(cinema.stats().contactOcclusionSamples,8);
assert.deepEqual(render({exterior:true}),{calls:11,world:2,clears:0});assert.deepEqual(render({exterior:true,reducedEffects:true}),{calls:6,world:2,clears:0});
assert.deepEqual(render({worldOnly:true,reducedEffects:true}),{calls:5,world:1,clears:0});
cinema.captureStingFrame();const frozen=cinema.stingSnapshot().target,serial=cinema.stingSnapshot().serial;cinema.contextLost();cinema.recoverContext();render({reducedEffects:true});assert.equal(cinema.stingSnapshot().target,frozen);assert.ok(cinema.stingSnapshot().serial>serial);assert.equal(cinema.stats().worldMsaaSamples,4);
console.log('PASS full/reduced/exterior/world-only render paths, stable target ownership, profile restoration and sting context recovery. Interior reduced mode removes ten fullscreen draws.');
