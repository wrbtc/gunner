...
let membrane=0,roots=0,inner=0;assembled.traverse(n=>{
 if(!n.isMesh)return;
 if(n.name==='Intact egg membrane'){
  membrane++;assert.equal(n.material.transparent,true);assert.equal(n.material.opacity,.42);assert.equal(n.material.depthWrite,false);
  assert.equal(n.material.roughness,.82);assert.equal(n.material.metalness,0);assert.ok(n.material.color.equals(shellMat.color));
 }
 if(n.name==='Intact egg roots'){
  roots++;assert.equal(n.material.transparent,false);assert.equal(n.material.opacity,1);assert.equal(n.material.depthWrite,true);
 }
 if(n.parent?.name==='Museum egg-maggot')inner++;
});
assert.equal(membrane,1);assert.equal(roots,1);assert.equal(inner,1);
assert.equal(shellMat.roughness,.2,'cached source material is unchanged');
...
dragon.dispose();
console.log('PASS: independent skeletons/bind matrices, flat shading, independent animation and idempotent owned-resource disposal; Intact egg has a translucent leather sac, opaque roots and enclosed maggot without cloning live nests; whole-object framing fills portrait and landscape views, and dragons face the camera');
