...
await check('first-start-timing-preserves-synchronous-audio-call-and-only-observes-once',()=>{
 ...
 const body=`let firstStartMeasured=false;const menuMusic={setActive(value){assertInactive(value);}},bootCompleted=true,graphicsReady=true,QA_MODE=true,loadingSnapshot=()=>({status:'ready'}),dom={start:{disabled:false},intro:{},hud:{classList:{add(){}}}},game={contextLost:false},APPROACH_END=.065,pilot={prime(){calls.push('prime');}},reset=()=>calls.push('reset'),playTracking={start(){calls.push('track');}},audio={start(){calls.push('audio');},setScene(){}},queenSampleCues={prime(){calls.push('queenPrime');}},updatePlane=()=>{},logEvent=()=>{},armReducedAuto=()=>{},startupMark=(...v)=>marks.push(v);${source};return start;`;
 const start=new Function('calls','marks','assertInactive',body)(calls,marks,value=>assert.equal(value,false));start({skipOpening:true});assert.deepEqual(calls,['reset','track','audio','queenPrime','prime']);start({skipOpening:true});assert.deepEqual(marks,[['first-start-reset','begin'],['first-start-reset','end'],['first-start-audio','begin'],['first-start-audio','end']]);assert.equal(calls.filter(x=>x==='audio').length,2);
});
