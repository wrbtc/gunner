...
await check('first-start-timing-preserves-synchronous-audio-call-and-only-observes-once',()=>{
 ...
 const start=new Function('calls','marks','assertInactive',body)(calls,marks,value=>assert.equal(value,false));start({skipOpening:true});assert.deepEqual(calls,['reset','track','audio','prime']);start({skipOpening:true});assert.deepEqual(marks,[['first-start-reset','begin'],['first-start-reset','end'],['first-start-audio','begin'],['first-start-audio','end']]);assert.equal(calls.filter(x=>x==='audio').length,2);
});
