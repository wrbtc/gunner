...
await check('pending-module-import-times-out-and-cleans-up',async()=>{const work=deferred(),errors=[],options=entryOptions({load:()=>work.promise,onError:e=>errors.push(e)}),promise=enterGame(options);await tick();options.clock.fire();assert.equal(await promise,'failed');assert.equal(errors[0].code,'PREPARATION_TIMEOUT');work.resolve();await tick();assert.equal(errors.length,1);assert.equal(options.events.count(),0);});
...
await check('blocked-module-cannot-win-after-overall-deadline',async()=>{let now=0;const errors=[],options=entryOptions({load:async()=>{now=100;},now:()=>now,timeoutMs:10,onError:e=>errors.push(e)});assert.equal(await enterGame(options),'failed');assert.equal(errors[0].code,'PREPARATION_TIMEOUT');assert.equal(options.clock.tasks.size,0);});
