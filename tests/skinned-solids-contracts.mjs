...
assert.match(manifest.note,/0\.54\.68/);
...
assert.match(note,/9d67a8c5fcdc4a5d8bbbdbae9f428e87a963a978682c0e572f62af28567e495e/);
...
for(const row of candidateRows){
 ...
 const manifestRow=manifest.files.find(item=>item.path===row.path);
 assert.deepEqual(manifestRow,{path:row.path,bytes:row.bytes,sha256:row.sha256});
 const bytes=readFileSync(new URL(row.path,root));
 const head=bytes.subarray(0,80).toString('utf8');
 if(head.includes('git-lfs.github.com')){
  assert.match(bytes.toString('utf8'),new RegExp(row.sha256));
  assert.match(bytes.toString('utf8'),new RegExp(String(row.bytes)));
 }else{
