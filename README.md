...
This is the **v0.54.83 runtime with the concept-24 / hive menu**. Near-bank combat creepers now use the Ember Hollow walk/run/climb/throw pack (dancers and Field Notes unchanged). The lava river now has a brighter crusted flow with fewer, stealthier cooling-skin rafts and hairline cracks, braided wall falls, visual-only bank lips and fall splashes, while upper walls stay cold rock. The menu keeps 16px between the life/bullet row and DEPLOY GUNNER. The Alien Field Guide withholds the prior identification stills on first paint, shows a branded preparing tile until VIEW 3D thumbs land, and fits each complete museum specimen inside its card. VIEW 3D uses the available screen area, fitting width, height and depth with a small border at 100% zoom. Dragon A faces the camera. Museum solids cover rimmers, plasma bugs, Ember Hollow creepers, Dragon A, Cinder Maw tanks, Dancing Creepers, Intact brood eggs, egg larvae, and egg shells. Intact egg uses a thin, translucent leather sac with solid opaque roots. The Field Notes v02 larva stays fully inside and is faintly visible through the membrane. Every specimen supports bounded wheel/trackpad, button, and keyboard zoom; horizontal drag turns it and vertical drag repositions it so details remain visible while zoomed. RESET VIEW restores its original position, rotation, and 100% zoom. Dragon A is a static, lazily loaded Field Notes specimen; repaired Cinder and larva rigs are presentation-only. Gameplay meshes, nests, AI, hitboxes, combat, and live sky dragons remain unchanged. Bank creepers and climbers still use the lava loco skinned mesh when
...
The v0.54.74 performance pass removes redundant MSAA from the browser canvas
while retaining the four-sample HDR world target. Once the opaque briefing
poster has been presented, heavyweight world update and presentation work rests
until the Field Guide, settings, deployment, reset, or WebGL recovery needs it.
Selected render-loop vectors are reused instead of allocated each frame.
...
This export is the v0.54.81 successor to the production concept-24 runtime,
