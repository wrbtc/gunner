...
This is the **v0.54.62 runtime with the concept-24 / hive menu**. The menu keeps 16px between the life/bullet row and DEPLOY GUNNER. The Alien Field Guide uses VIEW 3D museum solids for rimmers, plasma bugs, Ember Hollow creepers, Dragon A, Cinder Maw tanks, Dancing Creepers, egg larvae, and egg shells. Every specimen supports bounded wheel/trackpad, button, and keyboard zoom; RESET VIEW restores its original rotation and 100% zoom. Dragon A is a static, lazily loaded Field Notes specimen; Hatter's repaired Cinder and larva rigs are presentation-only. Gameplay meshes, nests, AI, hitboxes, combat, and live sky dragons remain unchanged. Bank creepers and climbers still use the lava loco skinned mesh when
...
This export is the v0.54.62 successor to the production concept-24 runtime,
