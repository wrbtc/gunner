...
This is the **v0.54.70 runtime with the concept-24 / hive menu**. The menu keeps 16px between the life/bullet row and DEPLOY GUNNER. The Alien Field Guide withholds the prior identification stills on first paint and uses VIEW 3D museum solids for rimmers, plasma bugs, Ember Hollow creepers, Dragon A, Cinder Maw tanks, Dancing Creepers, Intact brood eggs, egg larvae, and egg shells. Intact egg assembles the landed shell with the Field Notes v02 larva under a uniform opaque leather membrane, keeps the root tuft sealed, and holds the maggot fully inside as a thin-membrane silhouette. Every specimen supports bounded wheel/trackpad, button, and keyboard zoom; horizontal drag turns it and vertical drag repositions it so details remain visible while zoomed. RESET VIEW restores its original position, rotation, and 100% zoom. Dragon A is a static, lazily loaded Field Notes specimen; repaired Cinder and larva rigs are presentation-only. Gameplay meshes, nests, AI, hitboxes, combat, and live sky dragons remain unchanged. Bank creepers and climbers still use the lava loco skinned mesh when
...
This export is the v0.54.70 successor to the production concept-24 runtime,
