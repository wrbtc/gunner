...
This is the **v0.54.62 runtime with the concept-24 / hive menu**. The menu keeps 16px between the life/bullet row and DEPLOY GUNNER. The Alien Field Guide uses VIEW 3D museum solids for rimmers, plasma bugs, Ember Hollow creepers, Cinder Maw tanks, Dancing Creepers, Intact brood eggs, egg larvae, and egg shells when those LFS objects are present. Intact egg assembles the landed shell with the Field Notes v02 larva; Dragon A is an LFS pointer for Field Notes only. Repaired Cinder and larva rigs are presentation-only; gameplay meshes, nests, AI, hitboxes, and combat remain unchanged. Bank creepers and climbers still use the lava loco skinned mesh when
