...
import {poseCinderLeg} from './cinder-maw.js?v=054-87';
...
const VISUAL_SCALE=5.8,HIT_SCALE=2;
...
function* buildTankerBugs({actors,scene,centerAt,widthAt,bankMeshes,glowTexture,audio,planeVel,aimTarget,cinderModel}){
 ...
 for(const e of actors){
  ...
  const rig=cinderModel.clone();rig.root.scale.setScalar(VISUAL_SCALE);e.root.add(rig.root);e.cinderRig=rig;e.body=rig.body;e.head=rig.head||rig.body;e.front=rig.body;e.jaw=rig.jaw;e.jawBase=rig.jaw.rotation.x;e.mouthAnchor=rig.mouth;
  ...
  const glow=new THREE.Sprite(new THREE.SpriteMaterial({map:glowTexture,color:0xff951d,transparent:true,opacity:0,blending:THREE.AdditiveBlending,depthWrite:false}));glow.position.z=rig.forwardLocal?.z<0?-.025:.025;rig.mouth.add(glow);e.throatGlow=glow;
  ...
  // Collision remains on the established game-space volumes. The reworked
  // asset has a different authored unit scale, so visual scale cannot resize hits.
  e.sampleHitSpheres=()=>{e.root.updateWorldMatrix(true,true);const anchor=rig.body.getWorldPosition(V()),q=rig.body.getWorldQuaternion(new THREE.Quaternion()),flipZ=rig.forwardLocal?.z>0?-1:1;volumes.forEach(([v,r],i)=>{e.hitSpheres[i].center.copy(V(v.x,v.y,v.z*flipZ).multiplyScalar(HIT_SCALE).applyQuaternion(q).add(anchor));e.hitSpheres[i].radius=r*HIT_SCALE;});rig.mouth.getWorldPosition(e.hitSpheres[4].center);e.hitSpheres[4].radius=.85*HIT_SCALE;return e.hitSpheres;};
 ...
 function pose(e,time,target,dt){
  ...
  const heading=e.aimHeading;e.root.position.copy(p);e.root.rotation.set(0,0,0);e.baseY=p.y;rig.root.rotation.set(0,heading+(rig.headingOffset||0),0);rig.resetPose();
  ...
  rig.body.position.addScaledVector(rig.forwardLocal||V(0,0,-1),-recoil*.07);
  const restQ=rig.rest.get(rig.body).quaternion,sourceForward=(rig.forwardLocal||V(0,0,-1)).clone().applyQuaternion(restQ);
  const worldForward=(rig.forwardLocal||V(0,0,-1)).clone().applyQuaternion(rig.root.getWorldQuaternion(new THREE.Quaternion()));worldForward.y=Math.sin(pitch-recoil*.018);worldForward.normalize();
  const parentInverse=rig.body.parent.getWorldQuaternion(new THREE.Quaternion()).invert(),idleAim=worldForward.applyQuaternion(parentInverse);
  rig.body.quaternion.copy(new THREE.Quaternion().setFromUnitVectors(sourceForward,idleAim).multiply(restQ));rig.body.rotateZ(Math.atan(slope));
  if(engaged){
   ...
   for(let i=0;i<5;i++){
    ...
    const localAim=goal.sub(origin).normalize().applyQuaternion(rig.body.parent.getWorldQuaternion(new THREE.Quaternion()).invert());
    const look=new THREE.Quaternion().setFromUnitVectors(sourceForward,localAim).multiply(restQ).multiply(new THREE.Quaternion().setFromAxisAngle(V(0,0,1),Math.atan(slope)));
    rig.body.quaternion.slerp(look,spraying?1:e.rear);
   }
  }
  if(rig.motionBones){
   const gait=time*3.1+e.initialPhase,tail=Math.sin(gait*.72)*(.045+e.travel/51*.045),headNod=Math.sin(time*1.35+e.initialPhase)*.018;
   for(const [bone,amount]of [[rig.motionBones.tailBase,tail],[rig.motionBones.tailMid,tail*1.7],[rig.motionBones.tailTip,tail*2.35]])if(bone)bone.quaternion.copy(rig.rest.get(bone).quaternion).multiply(new THREE.Quaternion().setFromAxisAngle(up,amount));
   if(rig.motionBones.neck)rig.motionBones.neck.quaternion.copy(rig.rest.get(rig.motionBones.neck).quaternion).multiply(new THREE.Quaternion().setFromAxisAngle(V(1,0,0),headNod-e.rear*.035));
   if(rig.motionBones.head)rig.motionBones.head.quaternion.copy(rig.rest.get(rig.motionBones.head).quaternion).multiply(new THREE.Quaternion().setFromAxisAngle(V(1,0,0),-headNod*.55));
  }
  const pressure=wind?smooth(f,.25,1):spraying?1:0;
  rig.jaw.quaternion.copy(rig.rest.get(rig.jaw).quaternion).multiply(new THREE.Quaternion().setFromAxisAngle(V(1,0,0),(rig.forwardLocal?.z>0?1:-1)*(.035+pressure*.23+recoil*.045)));
  ...
  const mouthDirection=(rig.forwardLocal||V(0,0,-1)).clone().applyQuaternion(rig.mouth.getWorldQuaternion(new THREE.Quaternion())),stanceHeading=Math.atan2(-mouthDirection.x,-mouthDirection.z);
  for(const l of e.legs){
   const walking=!engaged&&e.state!=='suppressed',hip=l.upper.getWorldPosition(V()),reach=(l.upperLength+l.lowerLength)*VISUAL_SCALE*.94,sole=.052*VISUAL_SCALE;
   ...
   const mustStep=holdReach>(l.upperLength+l.lowerLength)*VISUAL_SCALE*.985||stale>1.8;
