...
export function createRimmers({scene,camera,game,centerAt,widthAt,bankMeshes,audio,aimTarget,progress,launch,logEvent,canAttack,activeCommitments,traceTerrain,postMaterial,captureFrame,reduced,effectsAllowed=()=>true,solid=null}){
 ...
 for(let pack=0;pack<RIMMER_ENCOUNTERS.length;pack++){const spec=RIMMER_ENCOUNTERS[pack];for(let slot=0;slot<spec.count;slot++){const side=spec.side,p=spec.p,model=createRimmerSkinnedModel(solid),root=model.root;root.name=`rimmer-${pack+1}-${slot+1}`;root.scale.setScalar(3);scene.add(root);const nodes=[];for(let i=0;i<=110;i++){const q=p+(-90+i)/3050;nodes.push(rim(q,side,spec.inset));}
 ...
 const warning=e.state==='windup'?clamp((time-e.warningAt)/3,0,1):0;e.jaws.forEach((j,i)=>j.rotation.y=(i?-.32:.32)+(i?-.11:.11)*warning);e.arms.forEach((a,i)=>a.rotation.x=Math.sin(gait*6+i)*.07-warning*.16);e.throat.scale.setScalar(.01+warning*.58);e.syncPose?.();
 // Imported legs land at their fitted neutral footprint. Legacy local-Z
 // overshoot fights this shorter anatomy when its root travels sideways.
 if(e.stepFeet){if(e.spawnAt!==null)e.stepFeet({gait,ground,dt});}
 else for(const l of e.legs){const cycle=((gait+l.i*.5+(l.side>0?.5:0))%1+1)%1,swing=cycle>.66;const phase=swing?(cycle-.66)/.34:cycle/.66,along=swing?mix(-1.9,1.9,T.MathUtils.smoothstep(phase,0,1)):mix(1.9,-1.9,phase),z=(l.i?4.6:-4.5)+along,localFoot=l.restFoot?l.restFoot.clone():V(l.side*5.9,0,z),world=localFoot.clone().applyMatrix4(e.root.matrixWorld);const desired=world.clone();if(!l.anchor){l.anchor=desired.clone();l.previousSwing=false;}if(swing){if(!l.previousSwing)l.swingStart=l.anchor.clone();world.copy(l.swingStart).lerp(desired,T.MathUtils.smoothstep(phase,0,1));l.anchor.copy(world);}else world.copy(l.anchor);world.y=ground.at(world.x,world.z);l.surface.copy(world);world.y+=.12+(swing?Math.sin(phase*Math.PI)*3.6:0);l.previousSwing=swing;const foot=e.root.worldToLocal(world.clone()),hip=V(l.side*1.05,4.8,l.i?1.5:-1.5),knee=hip.clone().lerp(foot,.48).add(V(l.side*1.15,2.2,0));e.poseLeg(l,hip,knee,foot);l.planted=!swing;}
 ...
 function update(time,dt){const p=progress();clock=time;for(const e of actors){
  ...
  if(e.spawnAt===null){
   ...
   e.spawnAt=time;e.stepFeet?.({gait:e.moveDistance/(e.gaitDistance||17.4),ground,dt:0,reset:true});e.emergence=1;e.root.visible=true;event('rimmer-arrival',{source:e.id,pack:e.pack,side:e.side,distance});beginWarning(e,time,distance);
  }
 ...
 function impact(owner){const queen=!!owner?.queenArm,profile=queen?'queen':'rimmer',allowed=effectsAllowed(),accepted=allowed?sting.impact(profile):false;event('rimmer-flash',{source:owner?.id,profile,duration:queen?QUEEN_STING_SECONDS:STING_SECONDS,damage:queen?8:10,accepted,suppressed:!allowed});return accepted;}
 ...
 function reset(){clock=0;sting.reset();events.length=0;ctx.clearRect(0,0,hud.width,hud.height);for(const e of actors){e.spawnAt=e.warningAt=null;e.emergence=e.travel=e.moveDistance=0;e.state='hidden';e.root.visible=false;e.beam.visible=false;e.pulses.forEach(m=>m.visible=false);e.resetPose?.();e.dead=false;e.hp=HP.rimmer;e.commitment=false;e.shotsFired=0;e.timer=0;e.root.rotation.set(0,e.side*Math.PI/2,0);for(const l of e.legs){l.anchor=null;l.swingStart=null;l.previousSwing=false;}const base=path(e,0);e.root.position.set(base.x,ground.at(base.x,base.z),base.z);}}
