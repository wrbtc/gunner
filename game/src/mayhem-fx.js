...
export function createMayhemFX({ scene, route, centerAt = p => route.getPointAt(clamp(p, 0, 1)), widthAt = () => 48, seed = 0xb12a57, rockMaterial = null } = {}) {
  ...
  function explosion(at, size = 16, kind = 'rock') {
    ...
    const light = lights[lightCursor++ % lights.length]; light.position.copy(at).addScaledVector(UP, 3); light.userData.life = .32; light.userData.max = .32; light.userData.power = 98 * size; light.intensity = light.userData.power;
  ...
  function update(dt, time = 0, planePosition = ZERO, quiet = false) {
    ...
    for (let i = 0; i < sparks.length; i++) {
      ...
      sparkSize[i] = s.size * Math.min(1, f * 5);
    }
    for (const name of ['iPosition', 'iColor', 'iSize', 'iVelocity', 'iAmbient']) sparkGeometry.attributes[name].needsUpdate = true;
    for (const light of lights) { light.userData.life = Math.max(0, light.userData.life - dt); light.intensity = (light.userData.power || 0) * Math.pow(light.userData.life / (light.userData.max || 1), 2.8); }
  }
  function randHeight(i) { return .5 + (i % 5) * .17; }
  function stats() { return { lastImpact, sparks: sparks.filter(s => s.life > 0).length, fragments: fragments.filter(f => f.life > 0).length, persistentWreck: fragments.slice(transientFragments).filter(f => f.life > 0).length, smoke: smoke.states.filter(s => s.life > 0).length, fire: fire.states.filter(s => s.life > 0).length, lights: lights.filter(l => l.intensity > 0).length, lightEnergy: lights.reduce((sum, light) => sum + light.intensity, 0), fireOpacity: fire.mesh.material.uniforms.uOpacity.value, impactProfiles: Object.keys(profiles), stagedBlast:true, directionalSparks:true, maximumBlastSmokeSeconds:4.5, crustRafts: crustCount, maxDrawCalls: 5, caps: { sparks: sparkCount, fragments: fragments.length, persistentWreck: persistentFragments, smoke: smoke.states.length, fire: fire.states.length, lights: lights.length } }; }
  ...
  return { group, update, impact, explosion, destroySiege, lavaSplash, reset, stats, dispose };
}
