...
export function buildGuideModel(id,{eggNests,plasmaBugs,cinderModel}){
 let root;
 if(id==='eggs')root=eggNests.guideModel();
 else if(id==='creepers'||id==='dancers')root=createBankGuideModel(id==='dancers');
 ...
 const frame=new THREE.Group(),pivot=new THREE.Group();frame.add(pivot);pivot.add(root);pivot.scale.setScalar(scale);root.position.sub(center);frame.name='Guide '+id;frame.rotation.y=id==='eggs'?.25:Math.PI+.4;
