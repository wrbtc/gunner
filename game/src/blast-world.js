...
export function createBlastWorld({scene,centerAt,widthAt,bankMeshes}){
 ...
 function update(time,plane,reduced){const t=reduced?0:time;for(let i=0;i<count;i++){const j=i*4,o=i*3;positions[o]=plane.x+((phase[j]*600+t*(18+phase[j+3]*12))%600)-300;positions[o+1]=8+phase[j+1]*190+Math.sin(t*.3+phase[j]*20)*2;positions[o+2]=plane.z+((phase[j+2]*900-t*8)%900+900)%900-600;}geo.attributes.position.needsUpdate=true;}
 return{root,update,stats:()=>({concretePieces:n,exposedRebars:k,ashParticles:count})};
}
