...
export function createBlastWorld({scene,centerAt,widthAt,bankMeshes}){
 ...
 function setReducedEffects(value){reducedEffects=!!value;applyAshCap(reducedEffects);slabs.visible=!reducedEffects;rebar.visible=!reducedEffects;}
 function update(time,plane,reduced){applyAshCap(!!reduced||reducedEffects);slabs.visible=!reducedEffects;rebar.visible=!reducedEffects;if(reduced||reducedEffects)return;const t=time;for(let i=0;i<count;i++){const j=i*4,o=i*3;positions[o]=plane.x+((phase[j]*600+t*(18+phase[j+3]*12))%600)-300;positions[o+1]=8+phase[j+1]*190+Math.sin(t*.3+phase[j]*20)*2;positions[o+2]=plane.z+((phase[j+2]*900-t*8)%900+900)%900-600;}geo.attributes.position.needsUpdate=true;}
 return{root,update,setReducedEffects,stats:()=>({concretePieces:n,exposedRebars:k,ashParticles:count,ashDrawn:ash.visible?count:0,ruinsVisible:slabs.visible,reduced:!ash.visible||!slabs.visible})};
}
