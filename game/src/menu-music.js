...
export function createMenuMusic({track,outcomes={},buttons,volume,music,doc,events,storage}){
 ...
 let active=true,wanted=true,playing=false,attempt=0;
 ...
 function play(){
  ...
  Promise.resolve(track.play()).catch(()=>{if(current===attempt){pause();}});
 }
 const toggle=()=>{wanted=!playing;if(wanted)play();else pause();};
 for(const button of buttons)button.addEventListener('click',toggle);
 const gesture=e=>{if(!buttons.some(button=>button.contains(e.target)))play();};
 ...
 for(const [name,media] of Object.entries(tracks)){
  media.addEventListener('error',()=>{if(media===track){pause();label('RETRY MUSIC');}});
