...
export const REPORT_BUILD='0.54.73';
...
async function openGuideViewer(button=null){
 if(!guideViewerPromise)guideViewerPromise=import('./field-guide-viewer.js?v=054-73').then(({createFieldGuideViewer})=>guideViewer=createFieldGuideViewer({dialog:$('fieldGuide'),provider:()=>guideModelProvider})).catch(error=>{guideViewerPromise=null;throw error;});
...
if(dialog&&trigger){
 ...
 for(const [id,name,tag,description,counter]of FIELD_GUIDE){
  ...
  if(id==='queen')portrait.dataset.src='./assets/field-guide/queen.png?v=054-73';
