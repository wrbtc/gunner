...
const MIN_ZOOM=.6,MAX_ZOOM=3,ZOOM_STEP=1.25;
...
export function createFieldGuideViewer({dialog,provider,modelIds=MODEL_IDS}){
 ...
 function ensureRenderer(){if(renderer)return;renderer=new THREE.WebGLRenderer({alpha:false,antialias:true,powerPreference:'low-power'});renderer.setPixelRatio(1);renderer.setSize(420,330);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.35;renderer.setClearColor(0x121b17);renderer.domElement.setAttribute('aria-label','3D creature model. Drag or use arrow keys to rotate; scroll or use plus and minus to zoom.');renderer.domElement.tabIndex=0;renderer.domElement.style.touchAction='none';
 ...
 function reset(){if(!current)return;current.root.rotation.set(0,current.initialY,0);current.zoom=1;syncZoom();if(tickRaf)return;schedule();}
 ...
 async function chooseSpecimen(){
  ...
  try{
   ...
   if(created){created.initialY=created.root.rotation.y;created.zoom=1;models.set(id,created);}
