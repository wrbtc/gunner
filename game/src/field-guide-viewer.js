...
export function createFieldGuideViewer({dialog,provider,modelIds=MODEL_IDS}){
 ...
 function renderModel(model,width,height){ensureRenderer();renderer.setSize(width,height);camera.aspect=width/height;const vertical=THREE.MathUtils.degToRad(camera.fov),fov=Math.min(vertical,2*Math.atan(Math.tan(vertical/2)*camera.aspect)),distance=(model.viewRadius||1.65)/Math.sin(fov/2);camera.position.set(0,distance*.105,distance);camera.lookAt(0,0,0);camera.updateProjectionMatrix();scene.add(model.root);renderer.render(scene,camera);scene.remove(model.root);draws++;}
 ...
 async function chooseSpecimen(){
  ...
  try{
   ...
   if(created){created.initialY=created.root.rotation.y;models.set(id,created);}
   current=model;error=null;title.textContent=id==='egg-maggot'?'Egg larva':id==='egg-shell'?'Egg shell':trigger.closest('[data-creature]').querySelector('h3').textContent;status.textContent='';stage.append(renderer.domElement);syncMotion();startTick();
 ...
 return{prepare,show,stats:()=>({models:[...models.keys()],draws,contexts:renderer?1:0,expanded:detail.open,current:current?.root.name||null,rotation:current?.root.rotation.toArray()||null,error,building,disposed,animating:!!tickRaf,motionPaused,activeClip:current?.activeClip||null}),dispose(){if(disposed)return;disposed=true;release();document.removeEventListener('visibilitychange',onVisibility);reduced.removeEventListener('change',onReduced);resize.disconnect();dialog.removeEventListener('close',release);detail.remove();}};
}
