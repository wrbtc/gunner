...
export function createFieldGuideViewer({dialog,provider}){
 ...
 function schedule(){if(detail.open&&current&&!frame&&!tickRaf)frame=requestAnimationFrame(draw);}
 function stopTick(){if(tickRaf)cancelAnimationFrame(tickRaf);tickRaf=0;lastTick=0;}
 function startTick(){
  if(tickRaf||typeof current?.tick!=='function')return;
  if(frame){cancelAnimationFrame(frame);frame=0;}
  lastTick=0;
  const loop=now=>{
   tickRaf=0;
   if(!detail.open||!current||typeof current.tick!=='function')return;
 ...
 function show(button){trigger=button;const card=button.closest('[data-creature]'),id=card.dataset.creature;title.textContent=card.querySelector('h3').textContent;stopTick();current=building?null:models.get(id)||null;stage.replaceChildren();status.textContent='';controls.hidden=!current;
 ...
 detail.addEventListener('close',()=>{stopTick();if(frame)cancelAnimationFrame(frame);frame=0;drag=null;current=null;stage.replaceChildren();trigger?.setAttribute('aria-expanded','false');if(dialog.open)trigger?.focus();});
 ...
 function release(){generation++;if(detail.open)detail.close();stopTick();if(frame)cancelAnimationFrame(frame);frame=0;drag=current=null;for(const model of models.values())model.dispose();models.clear();thumbs.clear();if(renderer){const old=renderer;renderer=null;old.dispose();old.forceContextLoss();old.domElement.remove();}scene=camera=null;error=null;}
