...
| `game/assets/dragon-fg-a.glb` | 64020728 | prefix `38fdb98e6c774ced` (full digest when the object is hashed) |
...
`hellWorld.skyActivity` alone. The build machine did not have the 64MB object,
so the gate is size `64020728` plus sha256 prefix `38fdb98e6c774ced`. Refresh
to the full digest when the object is hashed. Remesh is out of scope.
