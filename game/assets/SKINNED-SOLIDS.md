...
| `game/assets/cinder-maw-skinned-draft.glb` | 1004440 | `2af643fb2751ad128e6b06cf9a3244348220a4a58cb9884603e6cf9a6e13a1d4` |
...
Cinder Maw draft: armature `TankArmature`, clips `idle`, `SW_slow_walk`, and
`jaw_windup`. VIEW 3D plays `idle` only. Bones `body` / `jaw` / `cinder_mouth`
must survive. Do not remesh or retexture this draft.
...
Cinder Maw VIEW 3D plays `idle` only. Vein Ascetic VIEW 3D plays `ritual_idle`
only. Dragon A stays rest pose. Cloned materials, no combat mixers, no
live-actor mutation. The Queen stays ENLARGE. HERO is unchanged. Eggs use
`egg-shell-a` / `egg-maggot-a` when present. Soft-GPU stays off.
